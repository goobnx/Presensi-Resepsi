...
// Query untuk mengambil data kehadiran tamu sesuai dengan id_tamu
$sql_hadir = "SELECT nama_tamu, alamat, status, kehadiran, waktu FROM tamu WHERE kehadiran = 'Hadir' AND id_tamu <= 705 ORDER BY status DESC";
$sql_tidak_hadir = "SELECT nama_tamu, alamat, status, kehadiran, waktu FROM tamu WHERE kehadiran = 'Tidak Hadir' AND id_tamu <= 705 ORDER BY status DESC";
$sql_penambahan = "SELECT nama_tamu, alamat, status, kehadiran, waktu FROM tamu WHERE id_tamu >= 707 ORDER BY status DESC";

$result_hadir = $koneksi->query($sql_hadir);
$result_tidak_hadir = $koneksi->query($sql_tidak_hadir);
$result_penambahan = $koneksi->query($sql_penambahan);

// Membuat array untuk menyimpan data
$data_hadir = array();
$data_tidak_hadir = array();
$data_penambahan = array();
...
if ($result_penambahan->num_rows > 0) {
    while($row = $result_penambahan->fetch_assoc()) {
        $data_penambahan[] = array(
            'nama_tamu' => $row['nama_tamu'],
            'status' => $row['status'],
            'alamat' => $row['alamat'],
...
// Menambahkan sheet ketiga untuk data penambahan
$sheet_penambahan = new \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet($spreadsheet, 'Penambahan');
$spreadsheet->addSheet($sheet_penambahan, 2);

// Mengatur format judul tebal dan align center untuk sheet ketiga
$sheet_penambahan->getStyle('A1:F1')->applyFromArray($titleStyle);
$sheet_penambahan->setCellValue('A1', 'No.');
$sheet_penambahan->setCellValue('B1', 'Nama');
$sheet_penambahan->setCellValue('C1', 'Status');
$sheet_penambahan->setCellValue('D1', 'Alamat');
$sheet_penambahan->setCellValue('E1', 'Kehadiran');
$sheet_penambahan->setCellValue('F1', 'Waktu');

$row_num = 2;
foreach ($data_penambahan as $key => $row) {
    $sheet_penambahan->setCellValue('A' . $row_num, $key + 1);
    $sheet_penambahan->setCellValue('B' . $row_num, $row['nama_tamu']);
    $sheet_penambahan->setCellValue('C' . $row_num, $row['status']);
    $sheet_penambahan->setCellValue('D' . $row_num, $row['alamat']);
    $sheet_penambahan->setCellValue('E' . $row_num, $row['kehadiran']);
    $sheet_penambahan->setCellValue('F' . $row_num, $row['waktu']);
    $row_num++;
}
$sheet_penambahan->getStyle('A2:F' . ($row_num - 1))->applyFromArray($dataStyle);
$sheet_penambahan->getStyle('B2:B' . ($row_num - 1))->applyFromArray($leftAlignStyle);
$sheet_penambahan->getStyle('D2:D' . ($row_num - 1))->applyFromArray($leftAlignStyle);

// Mengatur lebar kolom otomatis untuk sheet ketiga
foreach (range('A', 'F') as $columnID) {
    $sheet_penambahan->getColumnDimension($columnID)->setAutoSize(true);
}
